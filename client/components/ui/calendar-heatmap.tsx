...
function CalendarHeatmap({
  ...
  // Create a map of dates to their weights for easy lookup
  const dateWeightMap = new Map(
    weightedDates.map((entry) => [entry.date.toDateString(), entry.weight])
  );

  // Custom day component to add tooltip with weight
  const DayWithTooltip = ({ date, ...dayProps }: any) => {
    const weight = dateWeightMap.get(date.toDateString());
    // If no weight is found, render a standard day
    if (weight === undefined) {
      return <span {...dayProps}>{new Date(date).getDate()}</span>;
    }
      ...
      <TooltipProvider>
        <Tooltip>
          <TooltipTrigger asChild>
            <button {...dayProps}>{new Date(date).getDate()}</button>
          </TooltipTrigger>
          <TooltipContent>
            <span>
              Articles on {date.toDateString()}: {weight}
            </span>
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
